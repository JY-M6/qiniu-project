...
🎥 **[在线观看项目演示 Demo 视频](https://YOUR_ONLINE_VIDEO_URL)**
...
- 🌐 **多功能气泡工具栏与翻译**：AI 消息气泡提供快捷悬浮栏，支持**一键复制、TTS 语音朗读**以及**中英互译模态窗**，支持中英文对称双向即时翻译。
- 🗂️ **本地历史会话管理**：侧边栏支持基于 `localStorage` 自动持久化管理多条历史对话，自动生成对话首图缩略图和首句文字预览，支持新增对话、清空记录与单条记录物理删除。
- 🎨 **高度个性化系统配置**：
  - 支持 **API 厂商快速预设**（通义千问、OpenAI、Google Gemini、火山引擎豆包及自定义配置）。
  - 支持**深浅色主题一键切换**、背景流光霓虹动画强度调节。
  - 动态调节全局字体大小、AI 打字机生成速度、以及摄像头分辨率（320x240 / 640x480 / 1280x720 动态码率调节）。
